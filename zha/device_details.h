...
#define lbed_model "Left"
#define rbed_model "Right"
#define temp_model "Temp"
#define light_model "MasterBed Light"
...
const attribute lbed_model_attr = BuildStringAtt(MODEL_ATTR, lbed_model, sizeof(lbed_model), ZCL_CHAR_STR);
const attribute rbed_model_attr = BuildStringAtt(MODEL_ATTR, rbed_model, sizeof(rbed_model), ZCL_CHAR_STR);
const attribute temp_model_attr = BuildStringAtt(MODEL_ATTR, temp_model, sizeof(temp_model), ZCL_CHAR_STR);
const attribute light_model_attr = BuildStringAtt(MODEL_ATTR, light_model, sizeof(light_model), ZCL_CHAR_STR);
const attribute resolution_attr = attribute(RESOLUTION_ATTR, v_res, sizeof(v_res), ZCL_SINGLE, 0x01); // Resolution (0.01v)
const attribute eng_unit_volts_attr = attribute(ENG_UNITS_ATTR, bac_net_volts, sizeof(bac_net_volts), ZCL_ENUM16, 0x01);

// I'm not using the Binary status flags, so use the same attribute everywhere to save on SRAM
const attribute binary_status_flag = attribute(BINARY_STATUS_FLG, one_zero_byte, sizeof(one_zero_byte), ZCL_MAP8, 0x01); // Status flags
// Also not using
const attribute out_of_service = attribute(OUT_OF_SERVICE, one_zero_byte, sizeof(one_zero_byte), ZCL_BOOL, 0x01); // Status flags
...
const attribute min_pv_attr = attribute(MIN_PV_ATTR, four_zero_byte, sizeof(four_zero_byte), ZCL_SINGLE, 0x01); // 0.0v
...
attribute lbed_occupied_attr[] = {
    {BINARY_PV_ATTR, one_zero_byte, 1, ZCL_BOOL}, // present value
    binary_status_flag};
...
attribute light_bool_attr[]{
    {CURRENT_STATE, one_zero_byte, 1, ZCL_BOOL}};
attribute light_level_attr[]{
    {CURRENT_STATE, one_max_byte, 1, ZCL_UINT8_T}};
attribute light_color_attr[] = {
    {ATTR_CURRENT_X, init_color_x, 2, ZCL_UINT16_T},             // CurrentX
    {ATTR_CURRENT_Y, init_color_y, 2, ZCL_UINT16_T},             // CurrentY
    {ATTR_CURRENT_CT_MRDS, two_zero_byte, 2, ZCL_UINT16_T, 0x01} // ColorTemperatureMireds, (Make this constant, since we don't use)
};
attribute lbed_analog_in_attr[] = {
    {BINARY_PV_ATTR, four_zero_byte, 4, ZCL_SINGLE}, // present value
    binary_status_flag,
    out_of_service,
    eng_unit_volts_attr,
    min_pv_attr,
...
attribute humid_attr[] = {{CURRENT_STATE, two_zero_byte, 2, ZCL_UINT16_T}};
